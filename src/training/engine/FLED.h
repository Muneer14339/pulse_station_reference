...
public:
	...
	bool isValidFrame(const cv::Mat &Img_G, string &message);
...
private:
    std::vector<cv::RotatedRect> pre_nms_ellipses; // Store pre-NMS ellipses
    std::vector<double> pre_nms_scores;             // Store pre-NMS scores
